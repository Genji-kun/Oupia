...
const UserHeaderBar: React.FC = () => {
    ...
    const { userInfoData, isFetchingUserInfo, expanded, setExpanded } = useMessageContext();
    ...
    return (
        <div className="p-2 w-full flex justify-between items-center shadow dark:shadow-black/20 z-20">

            {
                userInfoData &&
                <>
                    <Link href={`/profile/${userInfoData.username}`} className=" border-2 border-transparent rounded-lg p-2 pr-6 hover:bg-gray-100 dark:hover:bg-oupia-bg dark:hover:bg-background dark:hover:border-oupia-sub">
                        <div className="flex gap-x-4 items-center">
                            <Avatar className='w-12 h-12'>
                                <AvatarImage src={userInfoData.avatar} alt={userInfoData.fullName} />
                                <AvatarFallback className="font-semibold">{userInfoData.fullName && convert(userInfoData.fullName)}</AvatarFallback>
                            </Avatar>
                            <div>
                                <h2 className="font-semibold text-lg leading-none">{userInfoData.fullName && userInfoData.fullName}</h2>
