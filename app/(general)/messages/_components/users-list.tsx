...
import { useAuthToken } from '@/hooks/query';
import Hydration from '@/components/shared/Hydration';

const UsersList = () => {

    const { currentUser } = useSelector((state: any) => state.currentUserSlice);
    const { userQuery, searchUsers, isLoading } = useMessageSearchContext();

    const { dataAuthToken, isFetchingAuthToken } = useAuthToken();

    const [chatRooms, setChatRooms] = useState<any[] | undefined>();

    useEffect(() => {
        if (dataAuthToken && currentUser) {
            signInWithCustomToken(auth, dataAuthToken);
